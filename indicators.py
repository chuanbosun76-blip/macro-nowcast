...
US = [
    ...
    dict(id="us_cpi_yoy", name="美国CPI同比", short="美CPI同比", group="通胀", unit="%", freq="M", role="nowcast",
         source={"fred": "CPIAUCSL", "transform": "yoy"}, release="次月 10–15 日（BLS）",
    ...
    dict(id="us_cpi_mom", name="美国CPI环比", short="美CPI环比", group="通胀", unit="%", freq="M", role="nowcast",
         source={"fred": "CPIAUCSL", "transform": "mom"}, release="次月 10–15 日（BLS）",
    ...
    dict(id="us_core_cpi_yoy", name="美国核心CPI同比", short="美核心CPI", group="通胀", unit="%", freq="M", role="nowcast",
         source={"fred": "CPILFESL", "transform": "yoy"}, release="次月 10–15 日（BLS）",
    ...
    dict(id="us_nfp", name="美国非农新增就业", short="非农新增", group="就业", unit="千人", freq="M", role="nowcast",
         source={"fred": "PAYEMS", "transform": "diff"}, release="次月第一个周五（BLS）",
    ...
    dict(id="us_unrate", name="美国失业率", short="美失业率", group="就业", unit="%", freq="M", role="nowcast",
         source={"fred": "UNRATE", "transform": "level"}, release="次月第一个周五（BLS）",
    ...
    dict(id="us_gdp_qoq", name="美国实际GDP环比折年", short="美GDP", group="增长", unit="%", freq="Q", role="ref",
         source={"fred": "A191RL1Q225SBEA", "transform": "level"}, release="季后首月末（BEA，初值）", keywords=["美国GDP", "美国经济", "衰退"], theory="消费、投资、净出口与库存的季度合成；GDPNow 类高频拆分"),
    dict(id="us_retail_mom", name="美国零售销售环比", short="美零售环比", group="增长", unit="%", freq="M", role="nowcast",
         source={"fred": "RSAFS", "transform": "mom"}, release="次月中旬（Census）",
    ...
    dict(id="us_durable_mom", name="美国耐用品订单环比", short="耐用品订单", group="增长", unit="%", freq="M", role="nowcast",
         source={"fred": "DGORDER", "transform": "mom"}, release="次月下旬（Census）", keywords=["耐用品", "订单", "波音", "资本开支"], theory="飞机订单（波音）主导波动，核心资本品反映投资"),
    dict(id="us_umcsent", name="密歇根消费者信心", short="密歇根信心", group="增长", unit="", freq="M", role="ref",
         source={"fred": "UMCSENT", "transform": "level"}, release="当月中旬初值/月末终值", keywords=["消费者信心", "密歇根", "通胀预期"], theory="汽油价格、股市、通胀预期、政治周期"),
    # —— 地产 ——
    dict(id="us_houst", name="美国新屋开工（千套，折年）", short="新屋开工", group="地产", unit="千套", freq="M", role="nowcast",
         source={"fred": "HOUST", "transform": "level"}, release="次月中旬（Census）", keywords=["美国地产", "新屋开工", "房贷利率", "住房"], theory="房贷利率、建筑许可（领先）、天气、建筑商信心 NAHB"),
    ...
    # —— 货币利率 ——
    dict(id="us_fedfunds", name="联邦基金利率（月均）", short="联邦基金利率", group="货币利率", unit="%", freq="M", role="ref",
         source={"fred": "FEDFUNDS", "transform": "level"}, release="每日（美联储）", keywords=["美联储", "降息", "加息", "FOMC", "利率"], theory="FOMC 决议与点阵图、通胀与就业双目标"),
    ...
    dict(id="us_trade_balance", name="美国商品和服务贸易差额", short="美贸易差额", group="外贸", unit="亿美元", freq="M", role="persist",
         source={"fred": "BOPGSTB", "transform": "level", "scale": 0.01}, release="隔月初（Census/BEA）", keywords=["美国贸易", "逆差", "关税", "进口"], theory="关税前抢进口、美元、能源出口"),
]
